...
general of all array operations in JavaScript. You can solve almost any 
...
possible to show that both filter and map can be derived as special applications of reduce. 
...
The callback function accepts four arguments. The first argument is known as the accumulator, 
  which gets assigned the return value of the callback function from the previous iteration, 
    ...
    nd the fourth is the array upon which reduce is called.
...
function getRating(watchList) {
  ...
  .filter(filter => filter["Director"] === "Christopher Nolan")
  .map(filter => Number(filter.imdbRating))
  .reduce( (a, b) => a + b ) /
  watchList.filter(filter => filter["Director"] === "Christopher Nolan").length
